...
{
    public class Hospital_2 : Hub
    {

        private readonly IHospitalService _hospitalService;
        private readonly IHubContext<Hospital_1> _hospital_1;
        private readonly IHubContext<Dashboard> _dashboard;

        public Hospital_2(IHospitalService hospitalService, IHubContext<Hospital_1> hospital_1, IHubContext<Dashboard> dashboard)
        {
            _hospitalService = hospitalService;
            _hospital_1 = hospital_1;
        ...
        public override async Task OnConnectedAsync()
        {
             ...
             */
            if (await _hospitalService.ConnectAgentAsync(2, channelId))
            {
                await Clients.Client(connectionId).SendAsync("Connected", channelId);
            }
            else
            {
                Util.WaitingQueue.AddWaitingCall(connectionId, channelId, 2);
        ...
        public override async Task OnDisconnectedAsync(Exception exception)
        {
            ...
            if (waitingCall.Item3 != -1)
            {
                ...
                else
                {
                    ...
                    {
                        await _hospital_1.Clients.Group(channel_Id).SendAsync("Connected", channel_Id);
                    }
